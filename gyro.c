...
float gyroGetRate()
{
	...
	/* These are the kinds of syntax bugs that kill us: Previously: (problem was the ()'s around 5/4095, b/c not cast
	float gyroVoltage = (float)SensorValue(gyro.sensorPort) * (5/4095) / scaleFactor;
	writeDebugStreamLine("%f", (float)SensorValue(gyro.sensorPort) *5/4095/scaleFactor);
	*/
